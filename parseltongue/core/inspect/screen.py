"""Screen — consistency view with focus and filtering.

Like Lens is for structure, Screen is for health. Prepare a sample
on the bench, then screen it.
...
Usage::
    ...
    dx.focus("readme.")                # narrow to namespace → new Screen

Search is handled by ScreenSearchSystem — register as a scope
in the main search system, or use ``(scope screen ...)`` queries.
"""
...
@dataclass
class ScreenItem:
    """A single item in the screen — issue, warning, or dangling."""
    ...
    @classmethod
    def from_dict(cls, d: dict) -> "ScreenItem":
        return cls(
            name=d["name"],
            category=d["category"],
            type=d["type"],
...
class Screen:
    ...
    def __init__(self, items: list[ScreenItem], consistent: bool):
        self._items = items
        self._consistent = consistent
        self._search: "ScreenSearchSystem | None" = None

    @property
    def search_system(self) -> "ScreenSearchSystem":
        """Lazy-init search system."""
        if self._search is None:
            from .systems.screen import ScreenSearchSystem

            self._search = ScreenSearchSystem(self)
        return self._search
    ...
    def search(self, query: str) -> dict:
        """S-expression query against the screening search system."""
        return self.search_system.evaluate(query)

    @classmethod
    def from_report(cls, lc, result=None) -> "Screen":
    ...
    def danglings(self, kind: str | None = None, namespace: str | None = None) -> list[ScreenItem]:
        """Dangling definitions (consumed by nothing)."""
        return self._filter(category="dangling", kind=kind, namespace=namespace)

    def loader(self) -> list[ScreenItem]:
    ...
    def focus(self, namespace: str) -> "Screen":
        """Narrow to a namespace prefix — returns a new Screen."""
        filtered = [i for i in self._items if i.name.startswith(namespace)]
        has_issues = any(i.category == "issue" for i in filtered)
        return Screen(filtered, consistent=not has_issues)
    ...
    def summary(self) -> str:
        ...
        groups: dict[tuple[str, str], list[ScreenItem]] = {}
        for item in self._items:
        ...
        for (cat, typ), group in sorted(groups.items()):
            ...
            by_kind: dict[str, list[ScreenItem]] = {}
            for item in group:
    ...
    def incremental(self, diffs_to_patch: set[str], lc) -> "Screen":
        """Return new Screen patching only specific diffs.

        Keeps items from self except:
        - items whose name is in diffs_to_patch (re-screened)
        ...
        return Screen(merged, consistent=not has_issues)
    ...
    def from_dict(cls, d: dict) -> "Screen":
        items = [ScreenItem.from_dict(i) for i in d["items"]]
...
# ScreenSearchSystem moved to systems/screen.py
# Re-export for backwards compatibility
from .systems.screen import ScreenSearchSystem  # noqa: F401, E402
