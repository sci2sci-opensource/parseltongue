"""Tests for LoaderMorphism — file-aware namespace patching via NavList."""
...
class TestMainModule:
    def test_main_preserves_names(self):
        ...
        ds = ParseltongueLoaderMorphism.transform(source, is_main=True)
        assert names(ds) == ["x", "y"]

    def test_main_tags_source_file(self):
        source = '(fact x 42 :origin "test")'
        ds = ParseltongueLoaderMorphism.transform(source, source_file="main.pltg", is_main=True)
    ...
    def test_main_resolves_aliases(self):
        ...
        ds = ParseltongueLoaderMorphism.transform(
            source,
            is_main=True,
            module_name="main",
...
class TestNonMainModule:
    def test_namespaces_fact(self):
        source = '(fact x 42 :origin "test")'
        ds = ParseltongueLoaderMorphism.transform(source, module_name="m", is_main=False)
        assert ds[0].node.name == "m.x"
        assert str(ds[0].sentence.expr[1]) == "m.x"

    def test_namespaces_defterm(self):
        source = "(defterm y (+ x 1))"
        ds = ParseltongueLoaderMorphism.transform(source, module_name="pkg.mod", is_main=False)
    ...
    def test_names_to_modules_populated(self):
        source = "(fact x 1)\n(fact y 2)"
        n2m = {}
        ParseltongueLoaderMorphism.transform(source, module_name="m", is_main=False, names_to_modules=n2m)
        assert n2m == {"m.x": "m", "m.y": "m"}

    def test_names_to_lines_populated(self):
        source = "(fact x 1)\n(fact y 2)"
        n2l = {}
        ParseltongueLoaderMorphism.transform(source, module_name="m", is_main=False, names_to_lines=n2l)
...
class TestSymbolPatching:
    ...
    def test_ignores_keyword_args(self):
        source = '(fact x 42 :origin "test")'
        ds = ParseltongueLoaderMorphism.transform(source, module_name="m", is_main=False, known_names={"m.:origin"})
        assert ds[0].sentence.expr[3] == ":origin"

    def test_ignores_variable_symbols(self):
        source = "(axiom rule (implies ?x ?y))"
        ds = ParseltongueLoaderMorphism.transform(source, module_name="m", is_main=False, known_names={"m.?x"})
        body = ds[0].sentence.expr[2]
...
class TestReporting:
    def test_report_collects_resolutions(self):
        report = MorphismReport()
        source = "(defterm y (+ x 1))"
        ParseltongueLoaderMorphism.transform(
            source,
            module_name="m",
            is_main=False,
            known_names={"m.x"},
...
class TestContextPatching:
    def test_patches_context_key(self):
        source = "(context :file)"
        n2m = {}
        ds = ParseltongueLoaderMorphism.transform(source, module_name="m", is_main=False, names_to_modules=n2m)
...
class TestInverse:
    def test_inverse_produces_valid_sexp(self):
        source = '(fact x 42 :origin "test")\n(defterm y (+ x 1))'
        ds = ParseltongueLoaderMorphism.transform(source, is_main=True)
        result = ParseltongueLoaderMorphism.inverse(ds)
...
class TestMultiFile:
    def test_two_modules_no_collision(self, tmp_path):
        ...
        n2m = {}
        ds_a = ParseltongueLoaderMorphism.transform(
            mod_a,
            source_file=path_a,
            module_name="a",
            is_main=False,
        ...
        known = {"a.x"}
        ds_b = ParseltongueLoaderMorphism.transform(
            mod_b,
            source_file=path_b,
            module_name="b",
            is_main=False,
    ...
    def test_main_plus_imported_module(self, tmp_path):
        ...
        ds_main = ParseltongueLoaderMorphism.transform(
            main_src,
        ...
        ds_lib = ParseltongueLoaderMorphism.transform(
            lib_src,
