...
class TestQuoteVerifier(unittest.TestCase):
    ...
    def test_list_normalization(self):
        ...
        result_with_number = no_list_verifier.verify_quote(
            self.sample_document, "1. First point about something important. 2. Second point that"
        )
        result_without_number = no_list_verifier.verify_quote(
            self.sample_document, "First point about something important. Second point that"
        )
    ...
    def test_list_normalization_complex(self):
        ...
        self.assertTrue(
            result_spanning["verified"], "With list normalization, a quote spanning list items should match"
        )
        ...
        self.assertFalse(
            result_no_norm["verified"], "Without list normalization, a quote spanning list items should not match"
        )
        ...
        self.assertTrue(result_exact["verified"], "Without list normalization, exact quote with number should match")
        ...
        self.assertTrue(
            result_substring["verified"], "Standard substring matching should work regardless of normalization"
        )
    ...
    def test_hyphenation_normalization(self):
        ...
        self.assertTrue(
            result_no_hyphen["verified"]
        )  # This should still match as it doesn't include the hyphenated part

        result_no_hyphen_fail = no_hyphen_verifier.verify_quote(
            self.sample_document, "Another example of multilevel hyphenation"
        )
...
class TestQuoteVerifier2(unittest.TestCase):
    ...
    def test_stopword_removal(self):
        ...
        self.assertFalse(
            result_no_stopwords["verified"], "Without stopword removal, quote with missing stopwords should not match"
        )

        # With stopword removal, this should match
        result_with_stopwords = self.stopword_verifier.verify_quote(document, quote)
        self.assertTrue(
            result_with_stopwords["verified"], "With stopword removal, quote missing only stopwords should match"
        )
        ...
        self.assertFalse(
            result_custom["verified"], "With limited custom stopwords, quote missing non-stopwords should not match"
        )
        ...
        self.assertTrue(
            result_custom_match["verified"], "With custom stopwords, quote missing only those stopwords should match"
        )
    ...
    def test_extreme_stopword_case(self):
        ...
        # All stopwords
        stopword_verifier = QuoteVerifier(
            remove_stopwords=True, stopwords={"the", "quick", "brown", "fox", "jumps", "over", "lazy", "dog"}
        )
        ...
        self.assertFalse(
            result2["verified"], "WARNING: Removing crucial words like 'not' changes meaning but still matches"
        )
...
class TestRealCases(unittest.TestCase):
    ...
    def test_complex_molecule(self):
        ...
        quote = verifier.verify_quote(
            text,
            "'3-(4,5-Dimethylthiazol-2-y1)-2,5-diphenyltetrazolium bromide (MTT) assay and ELISA were used to determine cell viability and cell proliferation',",
        )
        self.assertTrue(quote["verified"])
...
class TestHyphensAndDashes(unittest.TestCase):
    ...
    def test_multiple_hyphen_types_in_one_sentence(self):
        ...
        result = self.verifier.verify_quote(doc, "anti-inflammatory drug first tested in 2019 is now FDA-approved")
        self.assertTrue(result["verified"])
